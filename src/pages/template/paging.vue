	...
	export default {
		...
		methods: {
			...
			async loadData(source) {
				if (source === 'init') { // 列表初始化
					this.pageNum = 1
					this.list = []
					this.loadStatus = 'loadmore'
					this.isEmpty = false
				}
				if (this.loadStatus === 'nomore' || this.loadStatus === 'loading') { // 防止重复加载
					return;
				}
				this.loadStatus = 'loading' // 正在加载中
				this.$api.methods().then(res => {
					console.log("获取到的列表", res.list)
					if (source === 'init' && res.list === 0) { // 初始化时列表为空
						this.isEmpty = true
					}
					if (res.list.length < this.pageSize) { // 如果分页数量不够, 则是  没有更多的状态
			...
			formatList(list) { // 格式化列表，可以打造自己自定义的数据
